...
public class UIParticleSystem : MonoBehaviour {
    ...
    private IEnumerator CoParticleFly(Image particle)
    {
        ...
        while (particleLifetime < Lifetime)
        {
            ...
            float x = Particle.bounds.size.x / Particle.bounds.size.y;
            x = x > 1.0f ? x : 1.0f;
            float y = Particle.bounds.size.y / Particle.bounds.size.x;
            y = y > 1.0f ? y : 1.0f;
            //set scale
            particle.transform.localScale = new Vector3(x, y, 0.0f) * SizeOverLifetime.Evaluate(particleLifetime / Lifetime) * Size;
